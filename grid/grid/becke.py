...
def becke_helper_atom( points, weights, radii, centers, select, order) :
    """ Computes the Becke weights for a given atom in a grid
        ...
        **Arguments:**
        ...
        centers :
            The Cartesian coordinates of the nuclei. Numpy array with
            shape (natom,3)

        select :
            The select atom for wich the Becke weights should be computed.
...
#   Precompute the alpha parameters for each atom pair
    ...
    for iatom in range( natom ) :
        for jatom in range ( iatom+1 ) :
            ...
            # to stay way from a ridiculous imbalance.
            if alpha > 0.45 :
                 alpha = 0.45
            elif alpha < -0.45 :
                alpha = -0.45
...
#   Precompute interatomic distances
    ...
    for iatom in range( natom ) :
        for jatom in range ( iatom+1) :
            atomic_dists[offset] = distanceBecke( centers[iatom], centers[jatom] )
            offset += 1
